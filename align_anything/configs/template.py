...
class HOMEWORK(Template):
    ...
    def format_preference_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        select_key = "response_"

        better_response_key = select_key + str(raw_sample["better_response_id"])
        worse_response_key = select_key + str(1-raw_sample["better_response_id"])

        better_text = (
            f'{self.system_prompt}'
            f"{self.user_prompt.format(input=raw_sample['prompt'])}"
            f"{self.assistant_prompt.format(output=raw_sample[better_response_key])}"
        )
        worse_text = (
            f'{self.system_prompt}'
            f"{self.user_prompt.format(input=raw_sample['prompt'])}"
            f"{self.assistant_prompt.format(output=raw_sample[worse_response_key])}"
        )

        return_dict = {
            'better_text': better_text,
            'worse_text': worse_text,
        }
        return return_dict
    ...
    def  check_equal(self, raw_sample: dict[str, Any]) -> bool:
        ...
        return raw_sample['response_0'] == raw_sample['response_1']

@register_template('HOMEWORK_BETTER')
class HOMEWORK_BETTER(HOMEWORK):
    def format_supervised_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        better_response_key =  "response_" + str(raw_sample["better_response_id"])
        prompt = (
            f'{self.system_prompt}'
            f"{self.user_prompt.format(input=raw_sample['prompt'])}"
        )
...
@register_template('HOMEWORK_WORSE')
class HOMEWORK_WORSE(HOMEWORK):
    def format_supervised_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        worse_response_key =  "response_" + str(1-raw_sample["better_response_id"])
        worse_response = raw_sample[worse_response_key] 
        worse_response = worse_response if worse_response else "NULL"
        prompt = (
            f'{self.system_prompt}'
            f"{self.user_prompt.format(input=raw_sample['prompt'])}"
        )
...
@register_template('HOMEWORK_DPO')
class HOMEWORK_DPO(HOMEWORK):
    def format_supervised_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        prompt = (
            f'{self.system_prompt}'
            f"{self.user_prompt.format(input=raw_sample['prompt'])}"
...
if __name__ == "__main__":
    ...
    dataset = load_dataset("/data/lmy/datas/PKU-SafeRLHF-single-dimension/")["train"]
    chattemplate = HOMEWORK_BETTER()
    print(chattemplate.format_supervised_sample(dataset[0]))
